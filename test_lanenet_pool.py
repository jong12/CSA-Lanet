...
def test_lanenet(image_path, weights_path, net_flag, use_gpu):
    ...
    net = lanenet_merge_model_pool.LaneNet(phase=phase_tensor, net_flag=net_flag)
        ...
        with tf.compat.v1.Session(config=sess_config, graph=g1) as sess:
            ...
            for epoch in range(epoch_nums):
                ...
                for index, binary_seg_image in enumerate(binary_seg_images):
                    ...
                    # mask_image = cluster.get_lane_mask(binary_seg_ret=binary_seg_image,instance_seg_ret=instance_seg_images[index])            #這邊的mask_image為黑背景但每條線已經上了不同顏色的圖(256*512)
                    ...
                    test = cv2.resize(binary_seg_image, (image_vis_list[index].shape[1],           #這邊的test為黑背景且線是白色的(binary)(720*1280原圖大小)
                                                         image_vis_list[index].shape[0]),interpolation=cv2.INTER_LINEAR)
                    ...
                    if save_dir is not None:
                        ...
                        # cv2.imwrite(image_save_path, mask_image) #instance segmentation 結果
                        cv2.imwrite(image_save_path, test*255) #binary segmentation 結果
            ...
            print('總預測車道線的部分平均每張耗時:',t_start2/((epoch_nums-1)*CFG.TRAIN.T))
